...
const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({ onLogout }) => {
    ...
    const [exportMessage, setExportMessage] = useState<{ success: boolean; message: string } | null>(null);
    ...
    const handleExport = async () => {
        ...
        try {
            const dataToExport = {
                daily: reservationsToShow,
                mileage: filteredMileageData,
            };
            const result = await googleScriptService.exportDataToSheets(dataToExport);
            if (result.success) {
                setExportMessage({ success: true, message: '성공적으로 Google Sheets로 내보냈습니다!' });
            } else {
                throw new Error(result.message || 'Unknown error');
            }
        } catch (error) {
            console.error("Failed to export data", error);
            setExportMessage({ success: false, message: `내보내기 실패: ${error instanceof Error ? error.message : '서버 오류'}` });
        } finally {
        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        {isExporting ? <Spinner/> : '📤'}
                        <span>Sheets로 내보내기</span>
                    </button>
                    <button onClick={onLogout} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">로그아웃</button>
                </div>
            </div>

            {exportMessage && (
                <div className={`mb-4 p-3 rounded-lg text-center ${exportMessage.success ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>
